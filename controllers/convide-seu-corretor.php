...
if($_POST){
    ...
    <table cellpadding="0" cellspacing="0" border="0" style="margin-bottom: 50px; border: 4px solid #0B3458; padding: 10px; width: 600px;">
        <tr>
            <td style="font-family: Arial; font-size: 13px;">
                Seu amigo '.$_POST['nome'].' lhe enviou uma mensagem através da Sua Focal Point:<br /><br />
                <a style="display:block;" href="'.URL.'" target="_BLANK"><img src="'.URL.'static/img/bt-entrar.gif" /></a>
            </td>
        </tr>
        <tr>
            <td style="font-family: Arial; font-size: 13px;">'.$_POST['mensagem'].'</td>
        </tr>
    </table>
...
<table width="960" style="background: url(http://www.suafocalpoint.com/new/static/img/bottom-email.jpg) repeat-x; text-align: justify; height: 105px; margin: 0 auto;" cellspacing="0" cellpadding="0" border="0">
    ...
    $mails = array($_POST['email_corretor']);

    if(email($text, 'Venha Participar da '.SITE, $mails))
        $msg = '<p style="float:none;color:red;">Obrigado!</p><p style="float:none;color:red;">Sua Indicação foi enviada com sucesso!</p><p style="float:none;color:red; margin-bottom:20px;">Esperamos ter seu corretor(a) em breve em nossa rede.</p>';
}

include('views/convide-seu-corretor.php'); 