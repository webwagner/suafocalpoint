...
$(document).ready(function(){
    ...
    $('.link-letra').click(function(){
        var letra = $(this).html();
        location.href = "<?php echo url();?>/solicitacoes-contato/letra/"+letra;
    })  
    
    $('#especialidade').change(function(){
        var esp = $(this).val();
        location.href = "<?php echo url();?>/solicitacoes-contato/especialidade/"+esp;
    })  
    ...
    $(".add-contato").click( function() {
        ...
        if(r){
            $.getJSON('add_del_contato.php',{type: 'add', id : id} ,function(txt){
                if(txt.valor == 'YES'){
                    jAlert(contato+" adicionado com sucesso!", 'Adicionar Contato');
    ...
    $(".del-contato").click( function() {
        var contato = $(this).attr('rel');
        var id = $(this).attr('id');
        jConfirm('<font color="red">Tem certeza que deseja negar a solicitação de '+contato+' ?</font>', 'Negar Solicitação', function(r) {
        if(r){
            $.getJSON('add_del_contato.php',{type: 'del', id : id} ,function(txt){
                if(txt.valor == 'YES'){
                    jAlert("O convite foi rejeitado.", 'Negar Solicitação');
                    setTimeout(window.location.href = window.location.href,5000);
                }
            })           
        }
...
<div id="content">
    ...
    <div class="page page-contatos">
        ...
        <div class="breadcumbs">
            <span>Solicitações de Contato <font>(<?php echo $total;?>)</font></span>
        </div>
